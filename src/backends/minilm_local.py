...
(e.g. ``/models/minilm-1.0.0``) so rolling the mirror is a path
change, not an in-place overwrite.
...
Why this backend, alongside labse-local:
  ...
  * ~2-3x faster on the same CPU (fewer transformer layers, smaller
    attention matrices).
  * 50+ languages covered vs LaBSE's 109 — narrower but still spans
    the 24 EU locales the platform surfaces.
...
Same async-load + optional dynamic-int8 quantise pattern as
LabseLocalBackend; if you tune one, tune both.
"""
...
@dataclass
class MinilmLocalBackend:
    # Directory containing the HuggingFace snapshot bytes for the mirrored
    # MiniLM revision. Must pre-exist on disk (placed there by the pod's
    # InitContainer); we never reach out to HF Hub from this backend.
    model_path: str
    # Signed-mirror identity, e.g. "minilm@1.0.0-e8f8c21". Stamped on every
    ...
    # toggle kept OFF by default for the same reason as LaBSE / NLLB
    # (torch.quantization eager mode grows RSS on some models).
    quantize: bool = False
        ...
        async with self._load_lock:
            ...
            if not os.path.isdir(self.model_path):
                raise BackendUnavailable(
                    f"minilm-local: model_path {self.model_path!r} does not "
            ...
            try:
                ...
                raise BackendUnavailable(
                    f"minilm-local: sentence-transformers/torch not installed ({exc})"
                ) from exc

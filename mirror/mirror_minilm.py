"""Download MiniLM at a pinned HF revision and produce a tarball ready to
`oras push`.

Called from the `mirror-minilm` workflow. Keeps Python where Python belongs
...
def _download_snapshot(repo: str, revision: str, dest: pathlib.Path) -> None:
    ...
    shipping both doubles the artifact size (~1.9 GB duplicate on MiniLM).
    """
...
def main() -> int:
    ...
    ap.add_argument(
        "--version-file",
        default=str(pathlib.Path(__file__).parent / "minilm.version.yaml"),
    )
    ap.add_argument(
        "--out-dir",
        default=None,
    ...
    encoder_id = f"minilm@{meta['mirror_version']}-{sha7}"
